<div align="center">

  <h2 dir="rtl">به نام خدا</h2>
  <img src="../assets/logo/logo.svg" width="180" style="margin: 0 auto; display: block;"/>

  <h2 dir="rtl">درس برنامه‌‌نویسی پیشرفته</h2>
  <h3 dir="rtl">فاز صفر و یک</h3>

  <p dir="rtl">
    <strong>دانشکده مهندسی کامپیوتر</strong><br/>
    <strong>دانشگاه صنعتی شریف</strong><br/>
    <strong>نیم‌سال دوم 05-04</strong>
  </p>

  <br/>

  <p dir="rtl">
    استاد:<br/>
    <strong>دکتر محمد امین فضلی</strong>
  </p>

</div>

<div class="page-break"></div>

# مقدمه

<div style="position: relative; display: flex; justify-content: center; align-items: center; margin: 20px auto;">
  <img src="assets/logo/pvz.png" alt="Frame" style="display: block; width: 50vw;  max-width: 100%;">
</div>

پروژه درس برنامه‌‌نویسی پیشرفته نسخه‌‌ای از بازی Plants vs. Zombies 2 خواهد بود.

در این بازی، بازیکن در یک دنیای خیالی و آخرالزمانی قرار دارد که توسط زامبی‌‌ها تصاحب شده است. مأموریت اصلی حفاظت از آخرین خطوط دفاعی انسان‌‌ها با استفاده از انواع گیاهان است. هر گیاه دارای توانایی‌‌های منحصربه‌‌فرد و اثرات ویژه‌‌ای است که بازیکن باید بر اساس نوع تهدید و شرایط مرحله‌‌ی بازی، استراتژی مناسبی را انتخاب کند.

<div style="position: relative; display: flex; justify-content: center; align-items: center; margin: 20px auto;">
  <img src="assets/images/pvz1.png" alt="Frame" style="display: block; width: 50vw;  max-width: 100%;">
  <img src="assets/images/pvz2.png" alt="Frame" style="display: block; width: 50vw;  max-width: 100%;">
</div>


# فهرست مطالب

- [ثبت‌‌نام](sections/02-implementation.md#register-menu)
- [ورود](sections/02-implementation.md#login-menu)
- [منوی اصلی](sections/02-implementation.md#main-menu)
- [منوی پروفایل](sections/02-implementation.md#profile-menu)
- [منوی بازی](sections/02-implementation.md#game-menu)
- [منوهای جانبی](sections/02-implementation.md#side-menus)
- [مکانیزم‌‌های اصلی](sections/03-fixed-stage-mechanisms.md#main-mechanisms)
- [گیاهان](sections/04-plants-food-upgrades.md#plants)
- [زامبی‌‌ها](sections/05-zombies-abilities.md#zombies)
- [فصل‌‌ها](sections/06-adventure-stages-bosses.md#stages)
- [گلخانه](sections/07-greenhouse-shop-settings.md#greenhouse)
- [کوئست‌‌ها](sections/08-quests.md#quests)
- [بازی امتیازی](sections/09-leaderboard-scoring.md#scoring-game)
- [لیدربورد](sections/09-leaderboard-scoring.md#leaderboard)
- [مینی‌‌گیم‌‌ها](sections/10-minigames.md#minigames)

<div class="page-break"></div>
